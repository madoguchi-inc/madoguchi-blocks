		...
		<li className="phone-cta__card is-open">
			...
			<div className="phone-cta__action">
				<span className="phone-cta__button phone-cta__button--balloon">
					<span className="phone-cta__balloon">
						{ texts.balloon }
					</span>
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const previewBanner =
		bannerPreset === CUSTOM_BANNER
			...
			: bannerPattern?.url
			? {
					url: bannerPattern.url,
					alt: bannerImageAlt || bannerPattern.alt,
			  }
			: null;
			...
			<InspectorControls>
					...
					{ bannerPreset !== '' && (
						<>
							<TextControl
								...
								onChange={ ( v ) =>
									setAttributes( { bannerImageAlt: v } )
								}
							/>
							<TextControl
								label={ __(
				...
				>
					<ToggleControl
						...
						help={ __(
							'PC は tel: リンクを押せないため、押下時に電話番号と QR コードのモーダルを開きます。SP はそのまま発信します。',
							'madoguchi-blocks'
						) }
						checked={ showPcModal }
						onChange={ ( v ) =>
